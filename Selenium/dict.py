...
# key跟key2陣列內容結合 
def ListsToDict(key_list, key2_list, value_list):
    # for i in range(len(key_list)):
    #     key_list[i]=key_list[i]+' ('+key2_list[i]+')'
    # print(len(key_list)) #10
    key_list = [key_list[i] + ' (' + key2_list[i] + ')' for i in range(len(key_list))]
    # print(len(key_list)) #10
    ...
    print(len(dict_list) == len(Result))
...
ListsToDict(key_list, key2_list, value_list)
